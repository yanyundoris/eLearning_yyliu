...
print np.mean(lac)
feature_coef = pd.DataFrame(feature_coef,columns=['page_number','charts_number','example_num','stt_accuracy',
                                 'stt_low_accuracy_ratio','flag','radio','ratio_change','word_count','animation',
                                 'hand_writing','speed'])

print np.mean(feature_coef)
#
feature_coef.to_csv('feature_coef_linear_update_label_2.csv',header=True, index=False)
...
feature_target = learning_df[['page_number','charts_number','example_num','stt_accuracy','stt_low_accuracy_ratio',
                                                       'flag','radio','ratio_change','word_count','animation','hand_writing','speed','Label1']].as_matrix(columns=None)
feature_target = np.array(feature_target, dtype=float).T
#
print feature_target.shape
#
core = np.corrcoef(feature_target)
np.savetxt('corrcoef_update_label_1.txt',core)
#
print np.corrcoef(feature_target)

feature_target = learning_df[['page_number','charts_number','example_num','stt_accuracy','stt_low_accuracy_ratio',
                                                       'flag','radio','ratio_change','word_count','animation','hand_writing','speed','Label2']].as_matrix(columns=None)
feature_target = np.array(feature_target, dtype=float).T
#
print feature_target.shape
#
core = np.corrcoef(feature_target)
np.savetxt('corrcoef_update_label_2.txt',core)
#
print np.corrcoef(feature_target)
